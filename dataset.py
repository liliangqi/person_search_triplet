...
class PersonSearchDataset:
    ...
    def get_query_im(self, query_name, pid):
        target_size = 600
        max_size = 1000
        pixel_means = np.array([[[102.9801, 115.9465, 122.7717]]])

        im = cv2.imread(osp.join(self.images_dir, query_name))
        im = im.astype(np.float32, copy=False)
        im -= pixel_means
        im_shape = im.shape
        im_size_min = np.min(im_shape[0:2])
        ...
        df = self.train_all.copy()
        # TODO: use pandas.query
        df = df[df['imname'] == query_name]
        df = df[df['flipped'] == False]
        df = df[df['pid'] == pid]
        assert df.shape[0] == 1
        gt_boxes = df.loc[:, 'x1': 'y2'] * im_scale
    ...
    def get_gallery_im(self, gallery_name, flipped):
        target_size = 600
        max_size = 1000
        pixel_means = np.array([[[102.9801, 115.9465, 122.7717]]])
